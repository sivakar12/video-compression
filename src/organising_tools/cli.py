import click
from .commands import video, dates, image, favorites, duplicates, folder

@click.group()
def cli():
    """All-in-one organising tool CLI."""
    pass

cli.add_command(video.compress_videos)
cli.add_command(dates.fix_modified_dates)
cli.add_command(dates.fix_created_dates)
cli.add_command(dates.add_timestamp_to_filename)
cli.add_command(dates.update_dates_from_filename)
cli.add_command(image.compress_photos)
cli.add_command(favorites.add_folder_to_favourites)
cli.add_command(favorites.go_to_favourite_folder)
cli.add_command(duplicates.find_duplicates)
cli.add_command(folder.group_by_month)
cli.add_command(folder.group_by_year)
cli.add_command(dates.change_filename_timezone)
